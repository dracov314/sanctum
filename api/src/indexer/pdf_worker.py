...
Own implementation.py. This module is the entry point
run inside the spawned child process created by ``_subprocess.extract_text_isolated``.
It deliberately imports nothing from the rest of the ``src`` package — only
``fitz`` (always needed) and, when OCR is configured, ``PIL``/``pytesseract`` —
so a throwaway extraction process never touches the DB engine or app state.

Keep the extraction logic in ``run`` behaviourally identical to
``_subprocess.extract_text_from_pdf`` (the in-process reference).
"""
...
def _ocr_settings() -> tuple[bool, str]:
    ...
    binary to be importable and runnable, probed lazily so an image without
    tesseract installed simply skips OCR.
    """
...
def run(filepath: str, text_only: bool = False) -> tuple[list[dict], bool]:
    ...
    Returns ``(pages, used_ocr)``: pages with an embedded text layer are read
    directly; text-less pages are OCR'd when OCR is available, otherwise
    skipped.

    When ``text_only`` is True, OCR is skipped entirely — only the embedded
    text layer is read. The deferred-OCR pipeline uses this for the fast scan
    phase: image-only pages come back missing (empty ``pages``), the caller
    queues the book for OCR, and OCR runs later page-by-page via ``ocr_page``
    so a slow scanned book never blocks or times out the main scan.
    """
    pages: list[dict] = []
    used_ocr = False
...
def ocr_page(filepath: str, page_index: int, languages: str, dpi: int | None = None) -> str:
    ...
    the parent checkpoints progress and resumes at the next page. Returns ""
    on any failure (page already has a text layer, render error, OCR error).

    ``dpi`` overrides the resolution the page is rasterized at; None uses the
    global ``OCR_DPI`` default.
    """
    doc = fitz.open(filepath)
    try:
        page = doc[page_index]
...
def _ocr_scale(dpi: int | None = None) -> float:
    """Rasterization scale for OCR (72 DPI x scale).

    Rendering is the CPU/memory-heavy half of OCR: a page's bitmap is
    ``(scale*width_pt) x (scale*height_pt) x 3`` bytes in memory. The default
    ``OCR_DPI=150`` (scale ~2.08) is a good accuracy/cost balance for
    Tesseract; lower it (e.g. 100) to cut render time and per-process memory on
    small hosts, raise it (e.g. 300) for cleaner OCR on faint scans at higher
    cost. An explicit ``dpi`` (per-book override) wins over the ``OCR_DPI`` env
    default.
    """
    if dpi is not None:
        try:
...
def main(filepath: str, result_path: str, text_only: bool = False) -> None:
    ...
    Writing to a file rather than a pipe/queue avoids the deadlock where a
    child blocks writing a large payload the parent hasn't drained. Any
    exception propagates and the process exits nonzero, which the parent
    treats as a crash — the result file is left empty so the parent can tell.
    """
...
def ocr_page_main(
    ...
    empty, which the parent treats as a failed page and skips. ``dpi``
    overrides the rasterization resolution (per-book re-OCR); None uses the
    global default.
    """
